...
Кейсы, отмеченные в CRM-influence галочкой «Показывать на сайте», публикуются кнопкой «Опубликовать на сайт»: CRM коммитит `cases.json` и фото в `assets/images/crm/`. `index.html` подгружает `cases.json` и добавляет эти кейсы к вписанным вручную. Эти файлы руками не правим - их перезапишет следующая публикация.
